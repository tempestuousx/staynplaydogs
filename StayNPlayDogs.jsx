...
const INITIAL_EMPLOYEES = [
  { id:"e1", name:"Sam D", role:"admin", pin:"1234", rate:22, email:"sam@staynplay.com" },
...
const INITIAL_SHIFT_SWAPS = [
  { id:"ss1", fromEmpId:"e3", toEmpId:"e2", dateFrom:dayKey(new Date(Date.now()+2*86400000)), dateTo:dayKey(new Date(Date.now()+4*86400000)), status:"pending-accept", adminApproval:"pending" },
];
...
export default function StayNPlayDogs() {
  ...
  const [currentUser, setCurrentUser] = useState("e1"); // logged-in employee id
  const [receipts, setReceipts] = useState([]); // [{ id, dogId, bookingId, date, type, basePrice, addons, addonTotal, lateFee, lateFeeReason, total, paidAt }]
  ...
  const [lateFees, setLateFees] = useState({}); // { bookingId: { amount: 25, reason: "Late pickup", notifiedAt: "..." } }
  const [pickupAlerts, setPickupAlerts] = useState([]); // [{ id, bookingId, dogName, ownerName, pickupTime, onShiftEmps:[], createdAt, dismissed }]
  const [pickupAlertedIds, setPickupAlertedIds] = useState(new Set());

  // Determine who is currently on shift
  const onShiftNow = useMemo(() => {
    const nowH = new Date().getHours(); const nowM = new Date().getMinutes();
  ...
  const onShiftNames = useMemo(() => [...new Set(onShiftNow)].map(id => employees.find(e => e.id === id)).filter(Boolean), [onShiftNow, employees]);

  // Auto-detect upcoming pickups and notify on-shift employees
  useEffect(() => {
    const checkPickups = () => {
      const active = bookings.filter(b => b.date === today && b.status === "checked-in" && b.pickupTime);
      active.forEach(b => {
        if (pickupAlertedIds.has(b.id)) return;
        const [h, m] = b.pickupTime.split(":").map(Number);
        const pickup = new Date(); pickup.setHours(h, m, 0);
        const diff = pickup - new Date();
        if (diff > 0 && diff < 7200000) {
          const dog = dogs.find(d => d.id === b.dogId);
          if (!dog) return;
          const empNames = onShiftNames.map(e => e.name.split(" ")[0]);
          const alert = {
            id: uid(), bookingId: b.id, dogName: dog.name, ownerName: dog.owner,
            pickupTime: b.pickupTime, onShiftEmps: empNames,
            createdAt: now(), dismissed: false,
          };
          setPickupAlerts(prev => [...prev, alert]);
          setPickupAlertedIds(prev => new Set([...prev, b.id]));
          setReminders(prev => [...prev, {
            id: uid(), message: "🐕 Pickup alert: " + dog.name + " is being picked up at " + b.pickupTime + " — Notified: " + empNames.join(", "),
            time: now(),
          }]);
          setLogs(prev => [{
            id: uid(), type: "system", dogId: b.dogId,
            detail: "Pickup at " + b.pickupTime + " — On-shift staff notified: " + empNames.join(", "),
            time: now(),
          }, ...prev]);
        }
      });
    ...
    checkPickups();
    const interval = setInterval(checkPickups, 30000);
    return () => clearInterval(interval);
  }, [bookings, dogs, pickupAlertedIds, onShiftNames]);
  ...
  const checkOut = useCallback((bookingId, amount=null, receiptDetail=null) => {
    setBookings(prev => prev.map(b => b.id === bookingId ? { ...b, status:"checked-out" } : b));
    const bk = bookings.find(b => b.id === bookingId);
    if (bk) {
      addLog(bk.dogId, "check-out", "Checked out" + (amount ? " — $" + amount + " charged" : ""));
      if (receiptDetail) {
        setReceipts(prev => [...prev, { id: uid(), dogId: bk.dogId, bookingId: bk.id, date: today, ...receiptDetail, paidAt: now() }]);
      }
    }
    closeModal();
  ...
  const notifyLatePickup = useCallback((dogId, lateAmount = 25) => {
    const bk = todayBookings.find(b => b.dogId === dogId && b.status === "checked-in");
    if (!bk) return;
    const dog = getDog(dogId);
    setLateFees(prev => ({ ...prev, [bk.id]: { amount: lateAmount, reason: "Late pickup — owner notified", notifiedAt: now() } }));
    addLog(dogId, "system", "Late pickup notification sent to " + (dog?.owner || "owner") + " — $" + lateAmount + " late fee applied");
    setLogs(prev => [{ id: uid(), type: "system", detail: "⚠ Late pickup: " + (dog?.name || "Dog") + " — owner " + (dog?.owner || "") + " notified, $" + lateAmount + " fee added", time: now() }, ...prev]);
  }, [todayBookings, getDog, addLog]);

  const getPickupInfo = useCallback((booking) => {
    if (!booking.pickupTime) return { soon: false, overdue: false, minutesLeft: null };
    const [h, m] = booking.pickupTime.split(":").map(Number);
    const pickup = new Date(); pickup.setHours(h, m, 0);
    const diff = pickup - new Date();
    const mins = Math.round(diff / 60000);
    return { soon: diff > 0 && diff < 7200000, overdue: diff <= 0, minutesLeft: mins };
  }, []);
  ...
  const Header = () => (
    <div style={{ background:C.surface, borderBottom:"1px solid "+C.border, padding:"0 16px", display:"flex", alignItems:"center", height:52, gap:16, position:"sticky", top:0, zIndex:100 }}>
      ...
      {(reminders.length > 0 || activePickupAlerts.length > 0) && <span style={{ ...badge(C.danger,"#fff"), cursor:"pointer", animation: activePickupAlerts.length > 0 ? "none" : "none" }} onClick={()=>{ const msgs = [...reminders.map(r=>r.message), ...activePickupAlerts.map(a=>"🐕 "+a.dogName+" pickup at "+a.pickupTime+" — Staff: "+a.onShiftEmps.join(", "))]; alert(msgs.join("\n\n")); setReminders([]); }}>🔔 {reminders.length + activePickupAlerts.length}</span>}
      <div style={{ display:"flex", alignItems:"center", gap:6, borderLeft:"1px solid "+C.border, paddingLeft:12 }}>
        <select style={{ border:"none", background:"transparent", fontSize:12, fontWeight:600, color:C.textMid, cursor:"pointer", outline:"none" }}
          value={currentUser} onChange={e=>setCurrentUser(e.target.value)}>
          {employees.map(e => <option key={e.id} value={e.id}>{e.name} {e.role==="admin"?"(Admin)":""}</option>)}
        </select>
        <EmpAvatar name={employees.find(e=>e.id===currentUser)?.name||"?"} size={30} />
      </div>
    </div>
  );
      ...
      <div style={{ ...card, display:"flex", gap:12, alignItems:"center" }}>
        ...
        <div style={{ flex:1, minWidth:0 }}>
          ...
          <div style={{ display:"flex", gap:4, marginTop:4, flexWrap:"wrap" }}>
            ...
            {(() => { const pi = getPickupInfo(booking); if (pi.overdue) return <span style={{...badge(C.danger,"#fff"), animation:"pulse 1.5s infinite", background:C.danger}}>🚨 OVERDUE — Pickup was {booking.pickupTime}</span>; if (pi.soon) return <span style={{...badge("#fff3e0","#e65100"), border:"1.5px solid #e65100"}}>⏰ Pickup in ~{Math.max(1,Math.round(pi.minutesLeft/60))}h ({booking.pickupTime})</span>; return null; })()}
            {booking.type==="boarding" && <span style={badge(C.boardingLight,C.boarding)}>☾ Boarding</span>}
            {lateFees[booking.id] && <span style={badge(C.dangerLight,C.danger)}>💰 Late Fee ${lateFees[booking.id].amount}</span>}
  ...
  // ─── DASHBOARD VIEW ────────────────────────────────────────────
  const dismissPickupAlert = (alertId) => setPickupAlerts(prev => prev.map(a => a.id === alertId ? { ...a, dismissed: true } : a));
  const activePickupAlerts = pickupAlerts.filter(a => !a.dismissed);

  const DashboardView = () => (
    <div style={{ padding:20 }}>
      {/* Pickup alert banner */}
      {activePickupAlerts.length > 0 && (
        <div style={{ marginBottom:16 }}>
          {activePickupAlerts.map(alert => (
            <div key={alert.id} style={{ background:"linear-gradient(135deg, #fff3e0, #ffe0b2)", border:"1.5px solid #e65100", borderRadius:10, padding:"12px 16px", marginBottom:6, display:"flex", alignItems:"center", gap:12, boxShadow:"0 2px 8px rgba(230,81,0,0.15)" }}>
              <span style={{ fontSize:22 }}>🔔</span>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700, fontSize:13, color:"#bf360c" }}>
                  Pickup Alert — {alert.dogName} at {alert.pickupTime}
                </div>
                <div style={{ fontSize:12, color:"#e65100", marginTop:2 }}>
                  Owner: {alert.ownerName} · Notified on-shift: <strong>{alert.onShiftEmps.join(", ") || "No staff clocked in"}</strong>
                </div>
              </div>
              <button style={{...btn("transparent","#bf360c",true), fontSize:16, padding:4}} onClick={()=>dismissPickupAlert(alert.id)}>✕</button>
            </div>
          ))}
        </div>
      )}
      ...
      {onShiftNames.length > 0 && (
        <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:12, fontSize:12, color:C.textMid }}>
          <span style={{ width:8, height:8, borderRadius:"50%", background:C.success, display:"inline-block" }} />
          On shift now: {onShiftNames.map(e => e.name.split(" ")[0]).join(", ")}
        </div>
      )}
  ...
  const EmployeeView = () => {
    ...
    const [newEmp, setNewEmp] = useState({ name:"", role:"staff", email:"", rate:"16", pin:"" });
    const isAdmin = employees.find(e => e.id === currentUser)?.role === "admin";
    const handleAddEmployee = () => {
      if (!newEmp.name.trim() || !newEmp.pin.trim() || newEmp.pin.length < 4) return;
      const emp = { id: uid(), name: newEmp.name.trim(), role: newEmp.role, pin: newEmp.pin, rate: parseFloat(newEmp.rate) || 16, email: newEmp.email.trim() || (newEmp.name.trim().split(" ")[0].toLowerCase() + "@staynplay.com") };
      setEmployees(prev => [...prev, emp]);
      setNewEmp({ name:"", role:"staff", email:"", rate:"16", pin:"" });
      setShowAddEmp(false);
    };
    ...
    const handleSwapRequest = () => {
      if (!swapDateFrom || !swapDateTo || swapFrom === swapTo) return;
      setShiftSwaps(prev => [...prev, { id:uid(), fromEmpId:swapFrom, toEmpId:swapTo, dateFrom:swapDateFrom, dateTo:swapDateTo, status:"pending-accept", adminApproval:"pending" }]);
      setSwapDateFrom(""); setSwapDateTo("");
    };
    ...
    const approveSwap = (swapId) => {
      ...
      setSchedule(prev => prev.map(s => {
        if (s.date===swap.dateFrom && s.empId===swap.fromEmpId) return {...s, empId:swap.toEmpId};
        if (s.date===swap.dateFrom && s.empId===swap.toEmpId) return {...s, empId:swap.fromEmpId};
        if (s.date===swap.dateTo && s.empId===swap.fromEmpId) return {...s, empId:swap.toEmpId};
        if (s.date===swap.dateTo && s.empId===swap.toEmpId) return {...s, empId:swap.fromEmpId};
        return s;
      }));
    };
      ...
      <div style={{ padding:20 }}>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:20 }}>
          <div>
            ...
            {/* Earnings - Admin Only */}
            {employees.find(e => e.id === currentUser)?.role === "admin" ? (
              ...
              <div style={{ marginTop:20 }}>
                <SectionHeader icon="💰" title="My Earnings" />
                <div style={{ ...card, display:"flex", gap:10, alignItems:"center" }}>
                  <EmpAvatar name={employees.find(e=>e.id===currentUser)?.name||"?"} />
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600, fontSize:12 }}>{employees.find(e=>e.id===currentUser)?.name}</div>
                    <div style={{ fontSize:11, color:C.textMid }}>Your estimated earnings this month</div>
                  </div>
                  <div style={{ fontWeight:700, fontSize:16, color:C.success }}>${calcEarnings(currentUser).toFixed(0)}</div>
                </div>
                <div style={{ fontSize:11, color:C.textLight, marginTop:4, fontStyle:"italic" }}>Only administrators can view all employee earnings.</div>
              </div>
            )}
          </div>
          ...
          <div style={{ borderLeft:"1px solid "+C.border, paddingLeft:16 }}>
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6, marginBottom:8 }}>
              <div style={{ display:"flex", flexDirection:"column", gap:4, background:C.bg, borderRadius:8, padding:8, border:"1px solid "+C.borderLight }}>
                <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                  <select style={{ ...input, flex:1 }} value={swapFrom} onChange={e=>setSwapFrom(e.target.value)}>
                    {employees.filter(e=>e.role!=="admin").map(e=><option key={e.id} value={e.id}>{e.name}</option>)}
                  </select>
                  <input style={{ ...input, flex:1 }} type="date" value={swapDateFrom} onChange={e=>setSwapDateFrom(e.target.value)} />
                </div>
                <div style={{ textAlign:"center", fontSize:11, fontWeight:700, color:C.textLight }}>↕ swaps with</div>
                <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                  <select style={{ ...input, flex:1 }} value={swapTo} onChange={e=>setSwapTo(e.target.value)}>
                    {employees.filter(e=>e.role!=="admin").map(e=><option key={e.id} value={e.id}>{e.name}</option>)}
                  </select>
                  <input style={{ ...input, flex:1 }} type="date" value={swapDateTo} onChange={e=>setSwapDateTo(e.target.value)} />
                </div>
              </div>
              <button style={{...btn(C.primary,"#fff",true), width:"100%", justifyContent:"center", padding:8, opacity:(!swapDateFrom||!swapDateTo||swapFrom===swapTo)?0.5:1}} disabled={!swapDateFrom||!swapDateTo||swapFrom===swapTo} onClick={handleSwapRequest}>Request Swap</button>
            </div>
            {shiftSwaps.map(sw => {
              const from = getEmp(sw.fromEmpId); const to = getEmp(sw.toEmpId);
              return (
                <div key={sw.id} style={{ ...card, padding:8 }}>
                  <div style={{ fontSize:11, fontWeight:600, marginBottom:2 }}>{from?.name} ↔ {to?.name}</div>
                  <div style={{ fontSize:10, color:C.textMid, display:"flex", gap:6 }}>
                    <span>{from?.name?.split(" ")[0]}: {fmtDate(sw.dateFrom)}</span>
                    <span>↔</span>
                    <span>{to?.name?.split(" ")[0]}: {fmtDate(sw.dateTo)}</span>
                  </div>
            ...
            {isAdmin && (
              <div style={{ marginTop:16, borderTop:"1px solid "+C.border, paddingTop:12 }}>
                <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:8 }}>
                  ...
                  <button style={btn(C.primary,"#fff",true)} onClick={()=>setShowAddEmp(!showAddEmp)}>{showAddEmp ? "Cancel" : "+ Add Employee"}</button>
                </div>
                {showAddEmp && (
                  <div style={{ ...card, padding:12, display:"flex", flexDirection:"column", gap:8, border:"2px solid "+C.primary+"44", background:C.primaryLight }}>
                    <input style={input} placeholder="Full Name *" value={newEmp.name} onChange={e=>setNewEmp({...newEmp, name:e.target.value})} />
                    <input style={input} placeholder="Email" value={newEmp.email} onChange={e=>setNewEmp({...newEmp, email:e.target.value})} />
                    <div style={{ display:"flex", gap:6 }}>
                      <select style={{ ...input, flex:1 }} value={newEmp.role} onChange={e=>setNewEmp({...newEmp, role:e.target.value})}>
                        <option value="staff">Staff</option>
                        <option value="admin">Admin</option>
                      </select>
                      <div style={{ position:"relative", flex:1 }}>
                        <span style={{ position:"absolute", left:8, top:"50%", transform:"translateY(-50%)", fontSize:12, color:C.textMid }}>$/hr</span>
                        <input style={{ ...input, paddingLeft:36, width:"100%" }} type="number" min="0" step="0.5" value={newEmp.rate} onChange={e=>setNewEmp({...newEmp, rate:e.target.value})} />
                      </div>
                    </div>
                    <input style={input} type="password" maxLength={4} placeholder="4-digit PIN *" value={newEmp.pin} onChange={e=>setNewEmp({...newEmp, pin:e.target.value.replace(/\D/g,"").slice(0,4)})} />
                    <button style={{...btn(C.primary,"#fff"), width:"100%", justifyContent:"center", padding:10, opacity:(!newEmp.name.trim()||newEmp.pin.length<4)?0.5:1}} disabled={!newEmp.name.trim()||newEmp.pin.length<4} onClick={handleAddEmployee}>Add Employee</button>
                  </div>
                )}
                <div style={{ display:"flex", flexDirection:"column", gap:4, marginTop:8 }}>
                  {employees.map(emp => (
                    <div key={emp.id} style={{ display:"flex", alignItems:"center", gap:8, padding:"6px 0", borderBottom:"1px solid "+C.borderLight }}>
                      <EmpAvatar name={emp.name} size={28} />
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:600, fontSize:12 }}>{emp.name}</div>
                        <div style={{ fontSize:10, color:C.textMid }}>{emp.email} · {emp.role}</div>
                      </div>
                      <span style={badge(emp.role==="admin"?C.primaryLight:C.successLight, emp.role==="admin"?C.primary:C.success)}>{emp.role}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={modal} onClick={closeModal}>
        <div style={{...modalContent, maxWidth:640}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{ padding:20, maxHeight:400, overflow:"auto" }}>
            {tab==="info" && !editing && (
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                <div style={{ border:"1px solid "+C.border, borderRadius:8, padding:12 }}>
                  <div style={{ fontWeight:700, fontSize:11, color:C.textMid, textTransform:"uppercase", marginBottom:4 }}>🔖 Microchip</div>
                  <div style={{ fontSize:13, fontFamily:"monospace", letterSpacing:1 }}>{dog.microchip || "Not recorded"}</div>
                </div>
                {dog.meds && (
                ...
                <div style={{ border:"1px solid "+C.border, borderRadius:8, padding:12 }}>
                  <div style={{ fontWeight:700, fontSize:11, color:C.textMid, textTransform:"uppercase", marginBottom:6 }}>👤 Owner</div>
                  <div style={{ fontSize:14, fontWeight:600, marginBottom:2 }}>{dog.owner}</div>
                  <div style={{ fontSize:12, color:C.textMid, marginBottom:8 }}>📞 {dog.phone||"No phone"} · 🆘 {dog.emergency||"No emergency"}</div>
                  <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                    {dog.phone && <a href={"tel:"+dog.phone} style={{...btn("#e3f2fd","#1565c0",true), textDecoration:"none", borderRadius:20, padding:"6px 14px"}}>📞 Call Owner</a>}
                    {dog.phone && <a href={"sms:"+dog.phone} style={{...btn("#e8f5e9","#2e7d32",true), textDecoration:"none", borderRadius:20, padding:"6px 14px"}}>💬 Text Owner</a>}
                    {dogBooking && dogBooking.status==="checked-in" && (() => {
                      const hasLate = lateFees[dogBooking.id];
                      return hasLate
                        ? <span style={{...badge(C.dangerLight,C.danger), padding:"6px 14px", fontSize:11}}>🚨 Late fee notified — ${hasLate.amount} at {fmt(hasLate.notifiedAt)}</span>
                        : <button onClick={()=>{if(window.confirm("Notify "+dog.owner+" of late pickup and apply $25 late fee?")) notifyLatePickup(dogId);}} style={{...btn(C.danger,"#fff",true), borderRadius:20, padding:"6px 14px"}}>🚨 Notify Late Pickup (+$25)</button>;
                    })()}
                  </div>
                  {dog.emergency && dog.emergency !== dog.phone && (
                    <div style={{ marginTop:8, paddingTop:8, borderTop:"1px solid "+C.borderLight }}>
                      <div style={{ fontSize:11, color:C.textMid, marginBottom:4 }}>Emergency Contact</div>
                      <div style={{ display:"flex", gap:6 }}>
                        <a href={"tel:"+dog.emergency} style={{...btn("#fff3e0","#e65100",true), textDecoration:"none", borderRadius:20, padding:"6px 14px"}}>📞 Call Emergency</a>
                        <a href={"sms:"+dog.emergency} style={{...btn("#fff3e0","#e65100",true), textDecoration:"none", borderRadius:20, padding:"6px 14px"}}>💬 Text Emergency</a>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {tab==="billing" && (() => {
              const dogReceipts = receipts.filter(r => r.dogId === dogId).sort((a,b) => new Date(b.paidAt) - new Date(a.paidAt));
              const totalSpent = dogReceipts.reduce((sum,r) => sum + (r.total||0), 0);
              return (
                <div>
                  {dogReceipts.length > 0 && (
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12, padding:"10px 12px", background:C.primaryLight, borderRadius:8 }}>
                      <span style={{ fontSize:12, fontWeight:600, color:C.primary }}>Total Billed ({dogReceipts.length} receipts)</span>
                      <span style={{ fontSize:18, fontWeight:800, color:C.primary }}>${totalSpent.toFixed(2)}</span>
                    </div>
                  )}
                  {dogReceipts.map(r => (
                    <div key={r.id} style={{ ...card, padding:12, marginBottom:8, borderLeft:"3px solid "+C.success }}>
                      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                        <span style={{ fontWeight:700, fontSize:13 }}>{fmtDate(r.paidAt)} — {r.type==="boarding"?"Boarding":"Daycare"}</span>
                        <span style={{ fontWeight:800, fontSize:15, color:C.success }}>${(r.total||0).toFixed(2)}</span>
                      </div>
                      <div style={{ fontSize:12, color:C.textMid }}>
                        <div style={{ display:"flex", justifyContent:"space-between" }}>
                          <span>Base ({r.type})</span><span>${(r.basePrice||0).toFixed(2)}</span>
                        </div>
                        {(r.addons||[]).length > 0 && (r.addons||[]).map(a => {
                          const opt = ADDON_OPTIONS.find(o=>o.key===a);
                          return opt ? <div key={a} style={{ display:"flex", justifyContent:"space-between" }}>
                            <span>{opt.label}</span><span>{opt.price>0?"+":""}{opt.price>0?"$"+opt.price:"-$"+Math.abs(opt.price)}</span>
                          </div> : null;
                        })}
                        {r.lateFee > 0 && (
                          <div style={{ display:"flex", justifyContent:"space-between", color:C.danger }}>
                            <span>Late Fee {r.lateFeeReason ? "("+r.lateFeeReason+")" : ""}</span><span>+${r.lateFee.toFixed(2)}</span>
                          </div>
                        )}
                      </div>
                      <div style={{ fontSize:10, color:C.textLight, marginTop:4 }}>Paid {fmt(r.paidAt)} · Card •••• 4242</div>
                    </div>
                  ))}
                  {dogReceipts.length === 0 && <div style={{ padding:24, textAlign:"center", color:C.textLight, fontSize:12 }}>No billing history yet. Receipts will appear here after checkout.</div>}
                </div>
              );
            })()}
            {tab==="history" && (
  ...
  const ActivityModal = () => {
    ...
    const [playPartners, setPlayPartners] = useState([]);
    ...
    const togglePlayPartner = (id) => setPlayPartners(prev => prev.includes(id) ? prev.filter(p=>p!==id) : [...prev, id]);
    const logPlayMulti = (good) => {
      const names = playPartners.map(id => getDog(id)?.name).filter(Boolean);
      if (names.length === 0) return;
      addLog(dog.id,"play",(good?"Played well with ":"Had issues with ")+names.join(", "));
      playPartners.forEach(partnerId => {
        const partner = getDog(partnerId); if(!partner) return;
        addLog(partnerId,"play",(good?"Played well with ":"Had issues with ")+dog.name);
      });
      closeModal();
    };
    const hasExistingFee = lateFees[booking.id];
    const applyLateFee = (amount) => {
      setLateFees(prev => ({...prev, [booking.id]: { amount, reason:"Late pickup", notifiedAt:now() }}));
      addLog(dog.id, "system", "Late pickup marked — $"+amount+" fee added to bill");
      setLogs(prev => [{ id:uid(), type:"system", detail:"⚠ Late pickup: "+dog.name+" — $"+amount+" fee auto-added to bill", time:now() }, ...prev]);
      closeModal();
    };
    return (
      <div style={modal} onClick={closeModal}>
          ...
          <div style={{ padding:20 }}>
            {screen==="main" && (
              <div>
                ...
                {/* Late Pickup Section */}
                {hasExistingFee ? (
                  <div style={{ background:C.dangerLight, border:"1.5px solid "+C.danger, borderRadius:10, padding:12, display:"flex", alignItems:"center", gap:10 }}>
                    <span style={{ fontSize:20 }}>🚨</span>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:700, fontSize:12, color:C.danger }}>Late Pickup Fee Applied</div>
                      <div style={{ fontSize:11, color:C.danger }}>{"$"+hasExistingFee.amount+" added to bill at "+fmt(hasExistingFee.notifiedAt)}</div>
                    </div>
                  </div>
                ) : (
                  <button style={{...btn(C.dangerLight,C.danger), width:"100%", padding:14, borderRadius:10, border:"1.5px solid "+C.danger, justifyContent:"center"}}
                    onClick={()=>setScreen("late")}>
                    <span style={{ fontSize:20 }}>🚨</span><span style={{ fontWeight:700 }}>Mark Late Pickup</span>
                  </button>
                )}
              </div>
            )}
            ...
            {screen==="play" && playType && <div>
              <button style={{...btn("transparent",C.textMid,true), marginBottom:8}} onClick={()=>{setPlayType(null);setPlayPartners([]);}}>← Back</button>
              <div style={{ fontSize:12, fontWeight:600, marginBottom:8, color:C.textMid }}>{playType==="good"?"Played well with...":"Had issues with..."} <span style={{ fontWeight:400, color:C.textLight }}>(select one or more)</span></div>
              <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:240, overflowY:"auto" }}>
                {otherCheckedIn.map(d=>{
                  const sel = playPartners.includes(d.id);
                  return (
                    <button key={d.id} style={{...card, display:"flex", gap:8, alignItems:"center", cursor:"pointer", border:sel?("2px solid "+(playType==="good"?C.success:C.danger)):("1px solid "+C.border), background:sel?(playType==="good"?C.successLight:C.dangerLight):"#fff", margin:0, transition:"all 0.15s"}} onClick={()=>togglePlayPartner(d.id)}>
                      <div style={{ width:22, height:22, borderRadius:6, border:sel?("2px solid "+(playType==="good"?C.success:C.danger)):("2px solid "+C.border), background:sel?(playType==="good"?C.success:C.danger):"#fff", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, transition:"all 0.15s" }}>
                        {sel && <span style={{ color:"#fff", fontSize:14, fontWeight:700, lineHeight:1 }}>✓</span>}
                      </div>
              ...
              {playPartners.length > 0 && (
                <button style={{...btn(playType==="good"?C.success:C.danger,"#fff"), width:"100%", padding:12, borderRadius:10, justifyContent:"center", marginTop:12, fontSize:14, fontWeight:700}}
                  onClick={()=>logPlayMulti(playType==="good")}>
                  {playType==="good"?"🎾":"⚠"} Log Play — {playPartners.length} {playPartners.length===1?"dog":"dogs"} selected
                </button>
              )}
            </div>}
            {screen==="late" && <div>
              <button style={{...btn("transparent",C.textMid,true), marginBottom:8}} onClick={()=>setScreen("main")}>← Back</button>
              <div style={{ fontWeight:700, fontSize:14, color:C.danger, marginBottom:4 }}>🚨 Mark Late Pickup</div>
              <div style={{ fontSize:12, color:C.textMid, marginBottom:12 }}>Select fee amount — this will be auto-added to {dog.name}'s checkout bill.</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
                {[15, 25, 35, 50].map(amt => (
                  <button key={amt} style={{...btn(C.dangerLight,C.danger), padding:16, borderRadius:10, border:"1.5px solid "+C.danger+"66", justifyContent:"center", flexDirection:"column"}}
                    onClick={()=>applyLateFee(amt)}>
                    <span style={{ fontSize:20, fontWeight:800 }}>{"$"+amt}</span>
                    <span style={{ fontSize:10, fontWeight:500 }}>{amt<=15?"Under 15 min":amt<=25?"15–30 min":amt<=35?"30–60 min":"60+ min"}</span>
                  </button>
                ))}
              </div>
              <div style={{ borderTop:"1px solid "+C.border, paddingTop:10 }}>
                <div style={{ fontSize:11, fontWeight:600, color:C.textMid, marginBottom:6 }}>CUSTOM AMOUNT</div>
                <div style={{ display:"flex", gap:6, alignItems:"center" }}>
                  <span style={{ fontSize:14, fontWeight:700, color:C.danger }}>$</span>
                  <input style={{...input, width:80}} type="number" min="0" step="5" placeholder="0" value={customFee} onChange={e=>setCustomFee(e.target.value)} />
                  <button style={btn(C.danger,"#fff",true)} onClick={()=>{const a=Number(customFee);if(a>0)applyLateFee(a);}}>Apply Fee</button>
                </div>
              </div>
            </div>}
          </div>
  ...
  const CheckoutModal = () => {
    ...
    const existingLate = lateFees[booking?.id];
    const [lateFee, setLateFee] = useState(existingLate ? existingLate.amount : 0);
    const [lateFeeReason, setLateFeeReason] = useState(existingLate ? existingLate.reason : "");
    if (!dog||!booking) return null;
              ...
              {!charged ? (
                ...
                <div style={{ textAlign:"center" }}>
                  ...
                  <button style={{...btn(C.primary,"#fff"), width:"100%", justifyContent:"center", marginTop:12}} onClick={()=>checkOut(booking.id, total.toFixed(2), {
                    type: booking.type, basePrice, addons: booking.addons||[], addonTotal, lateFee: Number(lateFee||0), lateFeeReason, total
                  })}>
                    Complete Checkout
                  </button>
                </div>
